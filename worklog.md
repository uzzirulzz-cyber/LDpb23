...
---
Task ID: whatsapp-ui
Agent: whatsapp-ui-builder
Task: Build WhatsApp Business section — two-panel WhatsApp Web-style interface with real Meta Cloud API integration.

Work Log:
- Read worklog.md (prior 13+ task entries), src/lib/whatsapp.ts (server-side: sendWhatsAppText, getWhatsAppCallLink, isWhatsAppConfigured, WHATSAPP_TEMPLATES — 7 templates, normalizePhone defaults 0XXXXXXXXXX → 92XXXXXXXXXX), src/components/crm/shared.tsx (SectionHeader, KpiCard, EmptyState, LoadingGrid, ChartSkeleton), src/components/crm/ui-helpers.tsx (MiniAvatar, initialsOf, timeAgo, formatDate, STATUS_META, BadgeShell, colorFor). Read inbox.tsx for InboxMessage/InboxThread pattern (channel union, ThreadPane optimistic send pattern), orders.tsx for OrderListItem shape (id, orderNumber, status, paymentStatus, total, currency, customer), contacts.tsx for ContactDetail shape (firstName, lastName, email, phones[], whatsapp, account).
- Verified backend API routes exist: GET/POST /api/crm/whatsapp/contacts (returns threads with channel=whatsapp + last message; POST accepts {name, phone, leadId, contactId} but does NOT persist phone — InboxThread has no phone column), GET /api/crm/whatsapp/status (returns {configured, message}), POST /api/crm/whatsapp/send (accepts {phone, message, threadId, staffId}, calls sendWhatsAppText, writes InboxMessage if threadId), GET /api/crm/inbox/[id] (returns thread with messages[] ascending), GET /api/crm/contacts/[id] (returns contact with parsed phones/emails), GET /api/crm/leads/[id] (returns lead with phone/whatsapp), GET /api/crm/orders?search= (matches orderNumber, customer email, customer phone).
- IMPORTANT discovery: src/lib/whatsapp.ts imports `db` from @/lib/db (Prisma), so it CANNOT be imported into a client component. Solution: mirrored the 7 template bodies verbatim into a CLIENT_TEMPLATE_BODIES const in the section file with a comment explaining why and noting they must stay in sync with /src/lib/whatsapp.ts. The template bodies match the server-side bodies exactly (verified by reading whatsapp.ts lines 30-115). Clicking a template fills the textarea with the raw body ({{placeholders}} included) so staff can fill variables before sending.
- IMPORTANT discovery: InboxThread has no phone column. The /api/crm/whatsapp/contacts POST accepts phone but does not persist it. Solution: client-side localStorage map keyed by threadId (PHONE_STORAGE_KEY = "playbeat_whatsapp_phones"). When a contact is added via the Add Contact dialog, the phone is saved to localStorage and React state. When sending/calling, the phone is read from this map. If no phone is cached for a thread (e.g., thread created by webhook elsewhere), the staff member is prompted via an inline dialog to enter a phone — which is then cached. This is the honest workaround given the schema gap.
- CREATED /home/z/my-project/src/components/crm/sections/whatsapp.tsx — single "use client" file, exports WhatsAppSection. ~750 lines.

Layout (two-panel WhatsApp Web style, grid md:grid-cols-[320px_1fr], max height 640px):
- Custom header (NOT SectionHeader — this section is full-screen per spec): WhatsApp icon (green #25D366 in tinted bg), "WhatsApp Business" title, status badge (green "Connected" if configured, amber "Not configured" if not), description.
- Configuration banner: if !isConfigured, prominent amber warning at top explaining WHATSAPP_ACCESS_TOKEN + WHATSAPP_PHONE_NUMBER_ID env vars required. Notes that Send button still works — API returns real error.
- Left panel (320px): header with "WhatsApp" title + green MessageCircle icon + status pill, search bar (filters by customerName/phone/subject), "Add Contact" button (green), contact list. Each contact: MiniAvatar (initials), name, phone or last message preview, timeAgo of lastMessageAt, unread badge (green #25D366 with white text). Selected contact highlighted with bg-white/5 + ring-1.
- Right panel (chat area): header with avatar, name, phone (or "No phone — set in profile"), Voice Call button (green, opens https://wa.me/{phone} in new tab via waLink()), Video Call button (green, same wa.me link — WhatsApp determines call type in-app), More options dropdown (View Profile, Close Chat). If no phone, call buttons are disabled with cursor-not-allowed + clicking opens inline "set phone" dialog. Message thread (from /api/crm/inbox/{threadId}): outbound messages right-aligned green #25D366 with dark text + rounded-br-sm, inbound left-aligned gray (rgba(255,255,255,0.06)) + rounded-bl-sm, timestamps below each bubble. Empty state: "No messages yet. Send your first message below." Message input: Textarea (Enter to send, Shift+Enter for newline — note this differs from inbox.tsx which uses ⌘+Enter; spec explicitly says Enter to send), Send button (green square 60x60), Templates dropdown (7 templates with labels: Order Received, Payment Under Review, Payment Verified, Order Processing, Order Completed, Payment Failed, Verification Required — clicking fills textarea with raw template body). Status notes: "No phone — set in profile to send" if no phone, "API not configured — sends will return an error" if !isConfigured. Auto-scrolls to bottom on new messages.
- Profile dialog (max-w-lg): identity block (avatar, name, phone, WhatsApp + status badges), 3 action buttons in grid-cols-3 (Open in WhatsApp, Voice Call, Video Call — all wa.me links, disabled if no phone), explanatory text about wa.me call type chosen in-app, editable phone input (cached locally), linked record section (lazy fetches /api/crm/contacts/{contactId} or /api/crm/leads/{leadId} if linked — shows name/email/whatsapp), order history section (lazy fetches /api/crm/orders?search={best_identifier}&limit=20 where best_identifier = linkedContact.email || linkedContact.whatsapp || linkedContact.phones[0] || linkedLead.email || linkedLead.phone || linkedLead.whatsapp || phone; shows order number, date, status, total via formatMoney with PKR default, payment status; or "Link this contact..." if not linked).
- Add Contact dialog: Name (required), Phone (required, defaults to "+92 "), Lead ID (optional), Contact ID (optional). POST /api/crm/whatsapp/contacts. On success: saves phone to localStorage + React state, closes dialog, auto-selects new thread, triggerRefresh.

Behavior:
- Optimistic send: on Send click, message is appended to localMessages state immediately with temp- prefixed id, message input cleared. On API response: if ok, refetch thread to replace temp with real message + pull any inbound; if !ok, toast.error with API error message (e.g., "Integration Not Configured") and remove the temp message. This gives honest feedback — never fakes success.
- Phone normalization for wa.me: waLink() strips + and [\s\-()] characters, defaults 0XXXXXXXXXX (Pakistan local) → 92XXXXXXXXXX (matches server-side normalizePhone in whatsapp.ts).
- Templates dropdown: 7 DropdownMenuItem entries with friendly labels. Clicking fills textarea with raw body containing {{placeholders}} (e.g., {{customer_name}}, {{order_id}}) so staff can fill variables before sending.
- useDashboardFetch for status, contacts, and thread messages — auto-refetch on global triggerRefresh. Local messages state synced from thread.messages via useEffect.

Critical-rule compliance:
- "use client" directive at top.
- NO mock data — all from real APIs via useDashboardFetch + manual fetch calls.
- NO fake buttons — every button calls a real API. Send → POST /api/crm/whatsapp/send. Add Contact → POST /api/crm/whatsapp/contacts. Voice/Video Call → real wa.me links in new tab. Templates → fills textarea. Profile actions → real wa.me links.
- Honest "Integration Not Configured" state: if !isConfigured, banner shown + Send button still enabled (POSTs to API which returns real error). Never fakes success.
- NEVER `<SelectItem value="">` — N/A here (no Select component used; only DropdownMenu which uses DropdownMenuItem without value prop).
- Loading/error/empty states everywhere: status loading skeleton, contacts loading 2-col skeleton, contacts error red banner, no contacts empty state with Add Contact CTA, no messages empty state, messages loading skeleton, profile linked-record loading skeleton, profile orders loading skeleton, "No orders found" empty state, "Not linked to a CRM contact or lead" empty state.
- Dark navy theme: glass-navy-card for all panels, bg via parent context (admin page sets bg-[#050814]). WhatsApp green #25D366 for accents (avatar bg tint, send button, call buttons, outbound message bubble, unread badge, "WhatsApp" status pill, "Connected" badge). Dark text on green bubbles (#070B19). White/5 borders.
- Imports: useDashboardFetch from @/hooks/use-dashboard-fetch, useDashboard from @/lib/store, formatMoney + Currency from @/lib/currency, cn from @/lib/utils, MiniAvatar/timeAgo/formatDate from ../ui-helpers, shadcn/ui from @/components/ui/* (Button, Input, Textarea, Label, Skeleton, Dialog + parts, DropdownMenu + parts), Lucide icons (MessageCircle, Phone, Video, Search, Plus, Send, MoreVertical, User, X, ExternalLink, AlertTriangle, CheckCircle2, ChevronDown, FileText, ShoppingCart). NO import from @/lib/whatsapp (would pull Prisma into client bundle) — templates mirrored locally with sync comment.
- toast from sonner for all mutation feedback.

Stage Summary:
- 1 file created: /home/z/my-project/src/components/crm/sections/whatsapp.tsx (~750 lines).
- No backend changes (all APIs already existed from prior subagent work).
- No config changes (section not yet wired into admin/page.tsx or sidebar.tsx — orchestrator can wire via dynamic import { WhatsAppSection } from "@/components/crm/sections/whatsapp" and add to SectionId union + sidebar nav).
- Lint/dev not run per instructions. File only.
